...
Adapted to double pendulum by: Matthys Jacobus Steynberg
"""
...
# input dimension
d = 1 # Using only one dimension of the double pendulum
...
dp_soln = solve_ivp(dummy, (0, maxtime), [0] , t_eval=t_eval, method='RK23')

# replace the data with the double pendulum data
def load_data(): 
    t_delay = 1
    _data=np.loadtxt('dp_data.txt')[:,1] #Use the y-coordinate of the second head
...
print('dp_soln shape: '+str(dp_soln.y.shape))
dp_soln.y[0,:] = data[:dp_soln.y.shape[1],0]

# total variance of Lorenz solution
total_var=np.var(dp_soln.y[0,:])

print('standard deviation for x,y,z: '+str(np.std(dp_soln.y[0,:])))
##
## NVAR
##
print(dp_soln)
...
for delay in range(k):
    for j in range(delay,maxtime_pts):
        x[d*delay:d*(delay+1),j]=dp_soln.y[:,j-delay]
...
u_predicted=x_test.reshape(-1,1)
u_data=data
...
plt.savefig('NGRC-Recur-Overlay_8.png')
...
plt.savefig('NGRC-Recur-Overlay_16.png')
...
plt.savefig('NGRC-Attractor.png')
...
ax0.plot(u_predicted[:pl_len3,0], u_predicted[8:pl_len3+8,0],u_predicted[16:pl_len3+16,0],lw=0.1,color='blue', alpha = 1)


ax0.xaxis.set_ticklabels([])
ax0.yaxis.set_ticklabels([])

ax0.plot(u_data[:pl_len3,0], u_data[8:pl_len3+8,0],u_data[16:pl_len3+16,0],lw=0.1,color='red', alpha = 0.9)
...
plt.savefig('NGRC-Attractor-Overlay.png')
...
plt.savefig('NGRC-Attractor-Overlay-Labels.png')
...
plt.savefig('NGRC-Autocor-Overlay.png')
