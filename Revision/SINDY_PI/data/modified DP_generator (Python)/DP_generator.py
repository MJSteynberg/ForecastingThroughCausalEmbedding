...
def ShowDoublePendulumAttractor(data, coord):
    ...
    y2=y2=l2*numpy.sin(theta2)+ y1     ### should it not be y2=-l2*numpy.cos(theta2) +y1    theirs is  y2=l2*numpy.sin(theta2)+y1
    fig = pylab.figure()
    ax0 = fig.add_subplot( projection='3d')
    i=8
    if coord == 'x1':
    
        ax0.plot(x1[1000:8000], x1[1008:8008],x1[1016:8016],lw=0.2,color='blue')
    elif coord == 'x2':
        ax0.plot(x2[1000:8000], x2[1008:8008],x2[1016:8016],lw=0.2,color='blue')
    elif coord == 'y1':
        ax0.plot(y1[1000:8000], y1[1008:8008],y1[1016:8016],lw=0.2,color='blue')  # first ones are 1000: 4000 in original code
    elif coord == 'y2':
        ax0.plot(y2[1000:8000], y2[1000+i:8000+i],y2[1000+2*i:8000+2*i],lw=0.2,color='blue')
        
    pylab.show()
    numpy.savetxt('Data/dp_data.txt', list(zip(x2, y2)))
    return y2
...
y2 = ShowDoublePendulumAttractor(data, 'y2')
