...
##### Load Data
    ...
    fig=plt.figure()
    ax=fig.add_subplot()
    ax.plot(data[0:1000,0],data[1:1001,0],'ko',markersize=0.1,color='red')
    ...
    train_len=3000    # Nuber of datasteps used in training
   ...
   #############################################
    ...
    # Saving Data
    #############################################
    np.savetxt('Predicted_Data/Lorenz/esn_delay_predicted.txt',u_predicted)
    np.savetxt('Predicted_Data/Lorenz/esn_delay_actual.txt',data[train_len+discard:train_len+discard+prediction_len])
...
try:
    spec = gridspec.GridSpec( nrows=3,ncols=1,
                             hspace=0.8,height_ratios=[1, 1, 1])
    fig=plt.figure()
    ...
    axs2.plot(range(1000),[x+1 for x in u_predicted[0:1000,0]],lw=0.5,color='darkblue')
    
    
    axs0.set_title('Actual',fontsize=15)
    axs1.set_title('Predicted',fontsize=15)
    axs2.set_title('When verticle axis is scaled to plot the log(1+actual value)',fontsize=15)
    axs2.set(yscale = 'log', ylim = (0.7, 10000))
    axs1.set(ylim = (-.23, .23))

    plt.savefig('Img/Lorenz-Coarse/ESN-Traj.png')
    plt.show()
    ####
    ...
    axs2.plot(u_data[S:S+pl_len3, 0],u_data[S+1:S+pl_len3+1,0],lw=0.06,color='red')
    axs2.set_title('Actual',fontsize=15)
    axs3.plot(u_predicted[S:S+pl_len3, 0],u_predicted[S+1:S+pl_len3+1,0],lw=0.06,color='blue')
    axs3.set_title('Predicted',fontsize=15)
    plt.savefig('Img/Lorenz-Coarse/ESN-Recur.png')
    plt.show()
    ####
    plt.rcParams['figure.figsize'] = [16,8]
    spec = gridspec.GridSpec( nrows=1,ncols=2,
    ...
    ax0.plot(u_predicted[:pl_len3,0], u_predicted[:pl_len3,1],u_predicted[:pl_len3,2],lw=0.05,color='blue')
    ...
    ax1.plot(u_data[:pl_len3,0], u_data[:pl_len3,1],u_data[:pl_len3,2],lw=0.05,color='red')
    ...
    plt.savefig('Img/Lorenz-Coarse/ESN-Attractor.png')
    plt.show()
    ...
    ax0.plot(u_data[:pl_len3,0], u_data[:pl_len3,1],u_data[:pl_len3,2],lw=0.05,color='red', alpha = 0.6)
    ax0.set_xlabel(r'$\theta(w(n))$',fontsize=10)
    ax0.set_ylabel(r'$\theta(w(n+1))$',fontsize=10)
    ax0.set_zlabel(r'$\theta(w(n+2))$',fontsize=10)
    ax0.xaxis.set_ticklabels([])
    ax0.yaxis.set_ticklabels([])
    ax0.zaxis.set_ticklabels([])
    
    plt.savefig('Img/Lorenz-Coarse/ESN-Attractor-Overlay.png')
    plt.show()
    ...
    axs.legend(['Actual','Predicted'], loc='upper left',fontsize=15)
    
    axs.tick_params(axis='x', labelsize=22)
    axs.tick_params(axis='y', labelsize=22)
    plt.savefig('Img/Lorenz-Coarse/ESN-Hist.png')
    plt.show()
    ...
    plt.savefig('Img/Lorenz-Coarse/ESN-Autocor.png')
    plt.show()
    ...
    plt.savefig('Img/Lorenz-Coarse/ESN-Autocor-Overlay.png')
    plt.show()
    ...
    plt.savefig('Img/Lorenz-Coarse/ESN-CDF.png')
    plt.show()
except:
